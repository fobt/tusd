...
[latest release](https://github.com/fobt/tusd/releases/latest).
...
```bash
git clone https://github.com/fobt/tusd.git
cd tusd
...
Those commands create two secrets which are used inside the example [docker-compose.yml](https://github.com/fobt/tusd/blob/main/examples/docker-compose.yml) file. The provided example assumes, that you also have a service named `minio` inside the same Docker Network.
